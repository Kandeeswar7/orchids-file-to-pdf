...
import { Upload, FileSpreadsheet, Check, ArrowRight, Loader2, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <p className="text-sm text-gray-400">Transform spreadsheets into beautiful PDFs with custom formatting</p>
      </motion.div>
      ...
      <motion.div
        ...
        className={cn(
          ...
          !file && !isDragging && "border-white/10 hover:border-white/20 hover:bg-white/5"
        )}
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        ...
        <div className="p-12 flex flex-col items-center justify-center text-center">
          ...
          {file ? (
            ...
            >
              ...
              <p className="text-sm text-emerald-400 font-medium">{(file.size / 1024).toFixed(1)} KB • Ready to convert</p>
            </motion.div>
          ) : (
            <div>
              <p className="text-white font-semibold text-lg mb-1">Drop Excel file here</p>
      ...
      >
        <div className="space-y-2">
          <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Orientation</label>
        ...
        <div className="space-y-2">
          <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Page Size</label>
          <div className="flex gap-2 glass p-1 rounded-xl">
            {['A4', 'Letter'].map((s) => (
              <motion.button
                key={s}
      ...
      <motion.button
        ...
        whileHover={file && !loading ? { scale: 1.02, boxShadow: "0 20px 40px rgba(16, 185, 129, 0.4)" } : {}}
