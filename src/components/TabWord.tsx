...
export function TabWord() {
  ...
  const handleFilesAdded = (newFiles: File[]) => {
    ...
    const maxFiles = plan === "premium" ? 10 : 1; // Example limit: 1 for free, 10 for premium
  ...
  const handleConvert = async () => {
    ...
    // A. Usage Reset (Must default to 0 if new day)
    checkUsageReset();

    const limits = PLAN_LIMITS[plan] || PLAN_LIMITS.free;
    const currentUsage = isNaN(dailyUsage) ? 0 : dailyUsage;

    // B. File Size Validation (Pre-check ALL files)
    ...
    // C. File Count Validation (Already checked in handleFilesAdded, but double-check)
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          // 3. Poll for Status
          const MAX_POLLING_TIME = 60000; // 60s Timeout
  ...
  return (
    <div className="space-y-8 relative">
      ...
      <motion.div
        ...
        onDrop={handleDrop}
        className={cn(
          "relative border-2 border-dashed rounded-2xl transition-all duration-300 cursor-pointer group overflow-hidden bg-black/20",
          isDragging && "border-blue-400 bg-blue-500/10 scale-[1.02]",
          files.length > 0 && !isDragging && "border-blue-500/50 bg-blue-500/5",
          files.length === 0 &&
            !isDragging &&
    ...
    Upgrade to upload multiple files.
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2 glass p-1 rounded-xl">
            {/* A4 ONLY */}
            <button className="flex-1 py-2.5 text-sm font-medium rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-lg cursor-default">
              A4 (Standard)
            </button>
          </div>
        </div>
